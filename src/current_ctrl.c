...
void current_ctrl_zcpi(int axis, float iq_ref, float id_ad, float iq_ad, float *vd_ref, float *vq_ref)
{
	...
	if(axis == 0){// X axis
	iq_ff = Cffi * xff_q[0] + Dffi * (double)iq_ref;						// input shaping
	xff_q[0] = Affi * xff_q[0] + Bffi * (double)iq_ref;

	id_er = 0.0 - (double)id_ad;										// dq-axis feedback
	iq_er = iq_ff - (double)iq_ad;
	*vd_ref = (float)(Cfbi * xfb_d[0] + Dfbi * id_er);
	xfb_d[0] = Afbi * xfb_d[0] + Bfbi * id_er;
	*vq_ref = (float)(Cfbi * xfb_q[0] + Dfbi * iq_er);
	xfb_q[0] = Afbi * xfb_q[0] + Bfbi * iq_er;
	}

	if(axis == 1){// Y axis
	iq_ff = Cffi * xff_q[1] + Dffi * (double)iq_ref;						// input shaping
	xff_q[1] = Affi * xff_q[1] + Bffi * (double)iq_ref;

	id_er = 0.0 - (double)id_ad;										// dq-axis feedback
	iq_er = iq_ff - (double)iq_ad;
	*vd_ref = (float)(Cfbi * xfb_d[1] + Dfbi * id_er);
	xfb_d[1] = Afbi * xfb_d[1] + Bfbi * id_er;
	*vq_ref = (float)(Cfbi * xfb_q[1] + Dfbi * iq_er);
	xfb_q[1] = Afbi * xfb_q[1] + Bfbi * iq_er;
	}
}
...
{
	xff_q[0] = 0.0; xfb_d[0] = 0.0; xfb_q[0] = 0.0;
	xff_q[1] = 0.0; xfb_d[1] = 0.0; xfb_q[1] = 0.0;
}
